...
//   const { user, isAuthenticated, loading: authLoading, getUserDisplayName } = useAuth()
...
//     // Fix: cast to unknown first to avoid TS overlap error
...
//       <div
//         key={m.id || i}
//         className={`flex items-end gap-2 mb-3 scrollbar-hide ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}
//       >
//         {m.role === 'assistant' && (
...
//     <div className="flex flex-col w-full bg-black text-white overflow-hidden relative"
...
//         <div ref={messagesRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-1 w-full">
...
export default function AskAI() {
  ...
  //   if (!isSpeechOutputSupported || !audioEnabled) return
  //   window.speechSynthesis.cancel()
  //   const utterance = new SpeechSynthesisUtterance(text.replace(/📊.*/, ''))
  //   utterance.lang = 'en-IN'
  //   utterance.rate = 1
