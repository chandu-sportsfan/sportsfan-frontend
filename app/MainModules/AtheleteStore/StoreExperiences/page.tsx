...
function ExperienceDetail({ exp, onClose }: { exp: typeof experiences[0]; onClose: () => void }) {
    ...
    return (
        <div className="flex-1 overflow-y-auto pb-[120px] no-scrollbar relative">
            ...
            {/* Bottom CTA - Changed from absolute to sticky */}
            <div className="px-4 py-4 border-t border-[rgba(255,255,255,0.06)] bg-[#0b0b0f] sticky bottom-0">
                {exp.soldOut ? (
                    ...
                    <div className="flex items-center gap-3">
                        <div className="flex-shrink-0">
                            ...
                            <p className="text-[18px] font-bold" style={{ background: 'linear-gradient(135deg,#c9115f,#cd620e)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                                ₹{(parseInt(price.replace(/[₹,]/g, '')) * seats).toLocaleString('en-IN')}
                            </p>
                        </div>
                        <button
    ...
    return (
        <div className="bg-black w-full flex justify-center min-h-screen">
            <div className="w-full max-w-6xl bg-[#0b0b0f] relative flex flex-col min-h-screen">
                ...
                <div className="sticky top-0 z-50 bg-[#0b0b0f] border-b border-[rgba(255,255,255,0.05)] h-[56px] flex items-center px-4 gap-3 flex-shrink-0">
                    <button
                        onClick={() => selectedExp ? setSelectedExp(null) : router.push('/MainModules/AtheleteStore')}
                        className="w-[36px] h-[36px] rounded-full bg-[rgba(255,255,255,0.06)] flex items-center justify-center flex-shrink-0"
                    >
                        <ArrowLeft className="w-[18px] h-[18px] text-white" />
                    </button>
                    <div className="min-w-0 flex-1">
                        <h1 className="text-white font-bold text-[17px] truncate">{selectedExp ? selectedExp.title : 'Experiences'}</h1>
                        {!selectedExp && <p className="text-[#99A1AF] text-[11px]">Exclusive access moments</p>}
                    </div>
                </div>
