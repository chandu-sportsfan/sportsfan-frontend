    ...
    >
      {src ? (
        <>
          ...
          />
          <div className="w-full h-full flex items-center justify-center" style={{ display: "none" }}>
            <span className="font-black text-gray-400 tracking-wider text-[8px]">{countryCode(name)}</span>
          </div>
        </>
      ) : (
        <span className="font-black text-gray-400 tracking-wider text-[8px]">{countryCode(name)}</span>
                ...
                {tabs.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-1.5 px-3 sm:px-4 py-2.5 rounded-xl text-[10px] sm:text-[11px] font-bold tracking-widest whitespace-nowrap transition-all flex-1 justify-center ${active ? "text-blue-400 border border-blue-500/40" : "text-gray-400 hover:text-white border border-transparent"
                        }`}
...
// ─────────────────────────────────────────────────────────────────────────────
  ...
  const isTbd = match.status === "tbd";
          ...
          return (
          <div
            className={`rounded-xl border p-2.5 flex flex-col gap-1.5 transition-all ${highlight ? "border-blue-500/50 shadow-[0_0_20px_rgba(37,99,235,0.15)]" : "border-white/10 hover:border-white/20"
              }`}
            style={{
