...
const ACTIVITY_TYPE_MAP: Record<string, ActivityKey> = {
  ...
  ROAR_POST:          "roarPost",
  ROAR_QUIZ:          "roarQuiz",
  ROAR_RAW_REACTIONS: "roarRawReactions",
};
...
const ACTIVITY_META: Record<ActivityKey, {
  ...
  fanBattleCreate: {
    action: "Created a Fan Battle", type: "Fantasy", icon: Gamepad2,
    color: "text-amber-400", hexColor: "#f59e0b",
    typeColor: "text-amber-400 border-amber-400/30 bg-amber-400/5",
  },
  trivia: {
  ...
  roarRawReactions: {
    action: "Posted ROAR Raw Reactions", type: "ROAR", icon: MessagesSquare,
    color: "text-sky-400", hexColor: "#38bdf8",
    typeColor: "text-sky-400 border-sky-400/30 bg-sky-400/5",
  },
...
function normalizeActivityKey(type: string, label: string): ActivityKey {
  ...
  if (value.includes("create battle") || value.includes("created a battle")) return "fanBattleCreate";
  ...
  if (value.includes("roar quiz"))                           return "roarQuiz";
  if (value.includes("raw reactions"))                       return "roarRawReactions";
