...
import Link from "next/link";
import React, { useEffect, useState, useCallback } from "react";
// import SpotlightPlayers from "@/components/Pulse/SpotlightPlayers";

// ─── Types ─────────────────────────────────────────────────────────────────────

interface PulseMeta { title: string; subtitle: string; summary: string; }
interface MatchTeam { name: string; short: string; score: string; overs: number; result: "won" | "lost"; }
interface MatchResult { match_number: number; date: string; venue: string; team1: MatchTeam; team2: MatchTeam; result: string; note: string; player_of_match: string; }
interface Tonight { match_number: number; teams: string; venue: string; time: string; context: string; badge: string; }
interface KPI { value: string; label: string; sub: string; color?: string; }
interface Quote { text: string; speaker: string; role: string; context?: string; sentiment?: string; badge?: string; badgeClass?: string; }
interface PointsRow { rank: number; team: string; played: number; won: number; points: number; form: string[]; playoff_zone: boolean; eliminated: boolean; color?: string; }
interface SearchItem { rank: number; query: string; heat: "hot" | "warm" | "normal"; }
interface FanTrend { rank: number; trend: string; sentiment?: string; badge?: string; badgeClass?: string; platform?: string[]; }
interface Ad { brand: string; product?: string | null; status?: string; tier?: string | null; detail: string; tierClass?: string; }
interface LangConsumption { rank?: number; language: string; share_pct: number; note: string; platform?: string; color?: string; gradientTo?: string; }
interface AdVolume { label: string; pct: number; color: string; }
interface Meme { rank: number; text?: string; description?: string; meta?: string; platform?: string; vibe?: string; verified_source?: string; }
interface MostLiked { rank?: number; text?: string; description?: string; meta?: string; source?: string; platform?: string[]; engagement?: string; accent?: string; }
interface Article { rank: number; source: string; tag: string; tagClass?: string; title: string; summary: string; url: string; linkColor?: string; accent?: string; }
interface Signal { num?: string; rank?: number; title: string; description?: string; desc?: string; accent?: string; theme?: string; teams?: string[]; }
...
/** Today's date in IST (UTC+5:30) as YYYY_MM_DD */
...
function buildUrl(dateUnderscore: string): string {
  return `https://res.cloudinary.com/dflnsufit/raw/upload/sf360/pulse/ipl_pulse_${dateUnderscore}.json`;
}

/** YYYY_MM_DD → YYYY-MM-DD */
function toISODate(s: string): string { return s.replace(/_/g, "-"); }
...
const AD_VOLUME_COLORS: Record<string, string> = {
  "Fintech/Ecomm": "#4a9eff", "FMCG/Auto": "#a87eff", "AI brands": "#ff6b2b", Others: "#555",
};
const SIGNAL_ACCENTS = ["#4a9eff", "#C41E1E", "#FFD84A", "#2bdd8c", "#a87eff"];
const ARTICLE_ACCENTS = ["#4a9eff", "#2bdd8c", "#FFD84A", "#ff6b2b", "#a87eff"];
const LIKED_ACCENTS = ["#a87eff", "#4a9eff", "#2bdd8c", "#FFD84A", "#C41E1E"];
...
function FilterBar({ searchText, onSearchChange, selectedDate, onDateChange, availableDates, onDateFileSelect, onClear, loading }: FilterBarProps) {
  return (
    <div className="flex flex-wrap gap-2 mb-5 md:mb-6">
      {/* Search */}
      <div className="relative flex-1 min-w-[180px]">
      ...
      {/* Archive date picker — only past dates */}
      ...
      {/* Clear */}
      {(searchText || selectedDate) && (
        <button onClick={onClear} className="flex items-center gap-1.5 bg-[#101016] border border-[rgba(255,255,255,0.09)] rounded-[20px] px-3 py-2 text-xs text-[#8888a2] hover:text-[#eeedf0] transition-colors">
          <X size={12} /> Clear
        </button>
      )}
...
export default function IPLPulse() {
  ...
  // ── 1. On mount: fetch today's JSON directly from Cloudinary ─────────────────
  const fetchToday = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const url = buildUrl(todayIST());
      const res = await fetch(url);
      if (!res.ok) throw new Error(`Today's report not available yet (${toISODate(todayIST())})`);
      setData(await res.json() as PulseData);
    } catch (e) {
      setError(e instanceof Error ? e.message : "Unknown error");
    } finally {
      setLoading(false);
  ...
  // ── 2. Fetch a past archive by its Cloudinary URL ────────────────────────────
  const fetchByUrl = useCallback(async (url: string) => {
    setLoading(true);
    setError(null);
    try {
  ...
  // ── 3. Background: populate date picker with past files (non-blocking) ───────
  const fetchAvailableDates = useCallback(async () => {
    try {
      const res = await fetch("/api/cloudinary/iplpulse?mode=list");
      const json = await res.json();
      ...
      // Exclude today — it's already shown by default
      const past = (json.files as PulseFileMeta[]).filter(
        f => !!f.reportDate && f.reportDate !== todayISO
      );
  ...
  useEffect(() => {
    fetchToday();
    fetchAvailableDates();
  }, [fetchToday, fetchAvailableDates]);

  // ── Clear → go back to today ──────────────────────────────────────────────────
  ...
  const handleDateFileSelect = useCallback((url: string) => fetchByUrl(url), [fetchByUrl]);

  // ── Filtered views ────────────────────────────────────────────────────────────
  const q = searchText.toLowerCase();
  const filteredSearches = data?.top_searches.filter(s => !q || s.query.toLowerCase().includes(q)) ?? [];
  const filteredSignals = data?.signals.filter(s =>
    !q || s.title.toLowerCase().includes(q) ||
    (s.description || s.desc || "").toLowerCase().includes(q) ||
    ...
    <div className="bg-[#09090e] min-h-screen mt-10 text-[#eeedf0] font-['DM_Sans',sans-serif] text-sm leading-relaxed">
      ...
      <div className="w-full max-w-[1300px] mx-auto px-3 sm:px-4 md:px-6 pb-12 md:pb-16">
            ...
            <div className="text-[11px] text-[#8888a2] mt-1 flex flex-wrap gap-1 items-center">
              {data ? (
                <>
                  ...
                  <span>·</span>
                  <span>{new Date(data.report_date + "T00:00:00").toLocaleDateString("en-IN", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</span>
                  {selectedDate && <><span>·</span><span className="text-[#ff6b2b] font-mono text-[9px] uppercase tracking-[0.1em]">Archive</span></>}
                </>
              ) : <Skeleton className="h-4 w-64" />}
            </div>
          </div>
        ...
        <FilterBar
          ...
          availableDates={availableDates} onDateFileSelect={handleDateFileSelect}
        ...
        {error && (
          <div className="bg-[rgba(196,30,30,0.1)] border border-[rgba(196,30,30,0.3)] rounded-xl p-4 mb-6 text-sm text-[#C41E1E] flex items-center justify-between">
            <span>{error}</span>
            <button onClick={fetchToday} className="text-xs underline ml-4">Retry</button>
          </div>
        )}
              ...
              <>
                  ...
                  <Card>
                    ...
                    <div className="flex flex-col gap-2.5">
                      {data.quotes.map((q, i) => {
                        ...
                        const accentMap: Record<BadgeClass, string> = { red: "#C41E1E", green: "#2bdd8c", gold: "#FFD84A", blue: "#4a9eff", orange: "#ff6b2b", neutral: "#50506a" };
                        return (
                          <div key={i} className="border-l-2 pl-3 py-2 bg-[#16161e] rounded-r-lg" style={{ borderLeftColor: accentMap[badgeCls as BadgeClass] || "#50506a" }}>
                            <p className="text-[12px] sm:text-[13px] text-[#eeedf0] leading-relaxed mb-1.5">&apos;{q.text}&apos;</p>
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="text-[10px] text-[#50506a] flex-1">{q.speaker}, {q.role}{q.context ? ` · ${q.context}` : ""}</span>
                              <Badge label={q.badge || q.sentiment || "insight"} cls={badgeCls} />
                            </div>
                          </div>
                        );
                      })}
                          ...
                          {data.points_table.map((row) => {
                            ...
                            return (
                              <tr key={row.rank} className={`hover:bg-[#16161e] ${row.eliminated ? "opacity-40" : ""} ${row.playoff_zone ? "bg-[rgba(196,30,30,0.02)]" : ""}`}>
                                ...
                                <td className="py-2 text-center border-b border-[rgba(255,255,255,0.07)]">
                                  <div className="flex items-center justify-center gap-[2px]">
                                    {row.form.map((f, fi) => <span key={fi} className={`inline-block w-2 h-2 rounded-[2px] ${f === "W" ? "bg-[#2bdd8c]" : "bg-[#C41E1E]"}`} />)}
                                  </div>
                                </td>
                              </tr>
                            );
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 mb-2">
                  <Card>
                    ...
                    {filteredSearches.length === 0
                      ...
                      : <div className="flex flex-wrap gap-1.5">
                        {filteredSearches.map((s) => {
                          const numCls = s.heat === "hot" ? "bg-[rgba(196,30,30,0.2)] text-[#C41E1E]" : s.heat === "warm" ? "bg-[rgba(255,216,74,0.15)] text-[#FFD84A]" : "bg-[#1c1c26] text-[#50506a]";
                          return (
                            <span key={s.rank} className="inline-flex items-center gap-1 bg-[#16161e] border border-[rgba(255,255,255,0.07)] rounded-[20px] px-2 sm:px-3 py-1 text-[11px] text-[#eeedf0]">
                              <span className={`font-mono text-[8px] rounded-full w-[16px] h-[16px] flex items-center justify-center flex-shrink-0 ${numCls}`}>{s.rank}</span>
                              {s.query}
                  ...
                  <Card>
                    ...
                    {data.fan_trends.map((t, i) => (
                      <div key={i} className={`flex flex-col sm:flex-row sm:items-center gap-2 py-2.5 ${i < data.fan_trends.length - 1 ? "border-b border-[rgba(255,255,255,0.07)]" : ""}`}>
                        ...
                        <div className="ml-auto sm:ml-0"><Badge label={t.badge || t.sentiment || "trend"} cls={t.badgeClass || inferBadgeClass(t.sentiment)} /></div>
                      </div>
                    ))}
                  </Card>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 mb-2">
                  <Card>
                    ...
                    <div className="flex flex-col gap-2">
                      {data.ads.map((ad, i) => {
                        const tierCls = ad.tierClass || (ad.status === "confirmed" ? "blue" : ad.status === "unconfirmed" ? "neutral" : "red");
                        return (
                          <div key={i} className="bg-[#16161e] border border-[rgba(255,255,255,0.07)] rounded-[10px] p-3">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-1.5 gap-1">
                              <div className="text-[12px] font-medium text-[#eeedf0]">{ad.brand}{ad.product ? ` · ${ad.product}` : ""}</div>
                              <Badge label={ad.tier || ad.status || "unknown"} cls={tierCls as BadgeClass} />
                            </div>
                            <div className="text-[11px] text-[#8888a2] leading-relaxed">{ad.detail}</div>
                          </div>
                        );
                ...
                {filteredArticles.map((a, i) => {
                  ...
                  return (
                    <div key={i} className="bg-[#101016] border border-[rgba(255,255,255,0.07)] rounded-xl px-3 sm:px-4 py-3 sm:py-4 relative overflow-hidden hover:border-[rgba(255,255,255,0.13)] transition-colors">
                      ...
                      <div className="text-xs sm:text-sm font-medium leading-snug mb-1.5 pl-2">{a.title}</div>
                      <div className="text-[11px] text-[#8888a2] leading-relaxed mb-2.5 pl-2">{a.summary}</div>
                      <a href={a.url} target="_blank" rel="noopener noreferrer" className="font-mono text-[9px] tracking-[0.06em] pl-2 hover:underline break-all" style={{ color: a.linkColor || accent }}>
                        READ FULL ARTICLE → {new URL(a.url).hostname.replace("www.", "")}
                      </a>
                    </div>
                  );
                ...
                {filteredSignals.map((s, i) => {
                  ...
                  return (
                    <div key={i} className="bg-[#101016] border border-[rgba(255,255,255,0.07)] rounded-xl px-3 sm:px-4 py-3 sm:py-4 flex flex-col sm:flex-row gap-2 sm:gap-3.5 relative overflow-hidden hover:border-[rgba(255,255,255,0.13)] transition-colors">
                      ...
                      <div>
                        <div className="text-xs sm:text-sm font-medium mb-1">{s.title}</div>
                        <div className="text-[12px] text-[#8888a2] leading-relaxed">{s.description || s.desc || ""}</div>
                        {s.teams && s.teams.length > 0 && (
                          <div className="flex gap-1 mt-2 flex-wrap">
                            {s.teams.map(t => <span key={t} className="font-mono text-[8px] px-1.5 py-[2px] rounded bg-[#16161e] border border-[rgba(255,255,255,0.07)] text-[#8888a2]">{t}</span>)}
                          </div>
                        )}
                      </div>
                    </div>
