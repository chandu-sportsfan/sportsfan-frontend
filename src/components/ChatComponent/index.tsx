...
const TypingIndicator = ({ name }: { name: string }) => (
  <div className="flex items-end gap-2 mt-2 mb-1">
    ...
    <div className="bg-[#181c24] rounded-[20px] rounded-bl-[5px] px-4 py-3 flex items-center gap-1.5 border border-[#c8705a]/[0.12] shadow-sm">
      <span
        className="w-1.5 h-1.5 bg-[#c8705a]/70 rounded-full animate-bounce"
...
const VoiceNotePlayer = ({
  ...
  const bars = Array.from(
    { length: 28 },
    (_, i) => 4 + Math.abs(Math.sin(i * 0.8 + 1.2) * 20),
  );
  return (
    <div className="flex items-center gap-3 min-w-[180px]">
      <button
        ...
        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all ${
          isMe
            ...
            : "bg-[#c8705a]/20 hover:bg-[#c8705a]/30"
        }`}
      >
        {playing ? (
      ...
      <span
        className={`text-[11px] flex-shrink-0 ${isMe ? "text-white/60" : "text-[#7a6a65]"}`}
      >
        {formatDuration(
          playing ? Math.round((progress / 100) * duration) : duration,
        )}
      </span>
    </div>
  );
...
const ReactionBadge = ({
  ...
  const topEmoji = Object.entries(reactions).sort(
    (a, b) => b[1] - a[1],
  )[0]?.[0];
  return (
    <div
      className={`absolute -bottom-3 flex items-center gap-0.5 bg-[#181c24] border border-[#c8705a]/[0.20] rounded-full px-1.5 py-0.5 shadow-lg cursor-pointer hover:scale-105 transition-transform ${isMe ? "left-2" : "right-2"}`}
    >
      <span className="text-[13px] leading-none">{topEmoji}</span>
      {total > 1 && (
        <span className="text-[10px] text-[#c8705a]/80 font-semibold">
          {total}
        </span>
      )}
    </div>
...
const Err = ({ msg, retry }: { msg: string; retry?: () => void }) => (
  <div className="flex items-center gap-2.5 bg-rose-950/30 border border-rose-500/20 text-rose-400 text-sm px-4 py-3 rounded-2xl mx-4 my-2">
    ...
    {retry && (
      <button onClick={retry} className="text-rose-300 text-xs underline">
        Retry
      </button>
    )}
  </div>
...
const UserPickerModal = ({
  ...
  const filtered = users.filter((u) => {
    const n =
      `${u.firstName ?? u.name ?? ""} ${u.lastName ?? ""}`.toLowerCase();
    return (
  ...
  return (
    <Modal onClose={onClose}>
      <div
        className="bg-[#111318] w-full max-w-[420px] rounded-3xl overflow-hidden shadow-2xl border border-[#c8705a]/[0.18]"
        style={{ maxHeight: "80vh" }}
      >
        <div className="flex items-center gap-3 px-5 py-4 border-b border-[#c8705a]/[0.08]">
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-[#c8705a]/[0.12] flex items-center justify-center hover:bg-[#c8705a]/[0.22] transition"
          >
            <X size={16} />
          </button>
        ...
        <div className="px-4 pt-3 pb-2">
          <div className="flex items-center gap-2.5 bg-[#c8705a]/[0.06] rounded-full px-4 py-2.5 border border-[#c8705a]/[0.12] focus-within:border-[#c8705a]/40 transition-colors">
            ...
            <input
              ...
              className="flex-1 bg-transparent outline-none text-sm text-white placeholder-[#4a3a35]"
            />
            {q && (
              <button onClick={() => setQ("")}>
                <X size={13} className="text-[#4a3a35]" />
              </button>
            )}
          </div>
        </div>
        ...
        >
          {loading && (
            <div className="flex items-center justify-center gap-2.5 py-10 text-[#7a6a65] text-sm">
              <Loader2 size={16} className="animate-spin" /> Loading…
            </div>
          )}
          {error && (
            <div className="px-5 py-4 text-rose-400 text-sm text-center">
              {error}
            </div>
          )}
          {!loading && !error && filtered.length === 0 && (
            <div className="px-5 py-10 text-center text-[#4a3a35] text-sm">
              No people found
            </div>
          )}
          {filtered.map((u, i) => {
            ...
            return (
              <div
                ...
                className="flex items-center gap-3.5 px-5 py-3 hover:bg-[#c8705a]/[0.05] cursor-pointer transition-colors active:bg-[#c8705a]/[0.10]"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {u.email && (
                    <div className="text-[12px] text-[#7a6a65] truncate">
                      {u.email}
                    </div>
                  )}
                </div>
    ...
    <div className="bg-[#111318] w-full max-w-[380px] rounded-3xl overflow-hidden shadow-2xl border border-[#c8705a]/[0.18]">
      <div className="flex flex-col items-center px-6 pt-7 pb-5 border-b border-[#c8705a]/[0.08]">
        ...
        <h3 className="mt-3 font-bold text-[18px] text-white flex items-center gap-1.5">
          {displayName}
          {chat.isVerified && (
            <BadgeCheck size={16} className="text-[#e8967e]" />
          )}
        </h3>
        {chat.isOnline ? (
          <span className="text-[12px] text-emerald-400 mt-0.5 flex items-center gap-1">
            <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full animate-pulse" />
            Active now
          </span>
        ) : chat.lastMessageAt ? (
          <span className="text-[12px] text-[#7a6a65] mt-0.5">
            Last active {timeAgo(chat.lastMessageAt)}
          </span>
        ) : null}
        <div className="flex gap-4 mt-4">
          {[
            ...
            >
              <div className="w-11 h-11 rounded-full bg-[#c8705a]/[0.12] flex items-center justify-center group-hover:bg-[#c8705a]/[0.22] transition text-[#c8705a]">
                {a.icon}
              </div>
      ...
      <div className="py-2">
        {[
          ...
          <button
            ...
            className={`w-full flex items-center gap-4 px-6 py-3 hover:bg-[#c8705a]/[0.05] transition-colors ${row.color}`}
          >
            {row.icon}
      ...
      <div className="px-5 pb-5 pt-1">
        <button
          onClick={onClose}
          className="w-full py-3 rounded-2xl bg-[#c8705a]/[0.08] text-gray-300 text-[14px] font-semibold hover:bg-[#c8705a]/[0.15] transition"
        >
          Cancel
        </button>
      </div>
  ...
  <Modal onClose={onCancel}>
    <div className="bg-[#111318] w-full max-w-[340px] rounded-3xl overflow-hidden border border-[#c8705a]/[0.18] shadow-2xl">
      <div className="px-6 pt-6 pb-4 text-center">
        ...
        <h3 className="font-bold text-[16px] text-white mb-1.5">
          Delete message?
        </h3>
        <p className="text-[13px] text-[#7a6a65] leading-relaxed">
      ...
      <div className="flex flex-col border-t border-[#c8705a]/[0.08]">
        {forEveryone && (
          <button
            ...
            className="py-4 text-[14px] font-semibold text-rose-400 hover:bg-rose-500/10 transition border-b border-[#c8705a]/[0.08]"
          >
            Delete for Everyone
          </button>
        )}
        <button
          ...
          className="py-4 text-[14px] font-semibold text-gray-300 hover:bg-[#c8705a]/[0.05] transition border-b border-[#c8705a]/[0.08]"
        >
          Delete for Me
        </button>
        <button
          onClick={onCancel}
          className="py-4 text-[14px] text-[#7a6a65] hover:bg-[#c8705a]/[0.04] transition"
        >
          Cancel
        </button>
      </div>
        ...
        {chats.map((chat) => {
          ...
          return (
            <div
              ...
              className="flex items-center gap-3.5 px-5 py-3 hover:bg-[#c8705a]/[0.05] cursor-pointer transition-colors"
            >
...
const PinnedBanner = ({
  ...
  <div
    className="flex items-center gap-2 px-4 py-2.5 bg-[#c8705a]/[0.07] border-b border-[#c8705a]/[0.12] cursor-pointer"
    onClick={onJump}
  >
    <Pin size={12} className="text-[#c8705a] flex-shrink-0 rotate-45" />
    <div className="flex-1 min-w-0">
      <p className="text-[10px] text-[#c8705a] font-semibold uppercase tracking-wider">
        Pinned
      </p>
    ...
    <button
      ...
      className="w-6 h-6 flex items-center justify-center hover:bg-[#c8705a]/[0.10] rounded-full transition"
    >
  ...
  <div className="flex gap-2 px-3 py-2 overflow-x-auto [&::-webkit-scrollbar]:hidden">
    {QUICK_REPLIES.map((r) => (
      <button
        key={r}
        onClick={() => onSelect(r)}
        className="flex-shrink-0 px-3.5 py-1.5 rounded-full bg-[#111318] border border-[#c8705a]/[0.18] text-[12px] text-gray-300 hover:bg-[#181c24] hover:border-[#c8705a]/35 transition-all"
      >
        {r}
      </button>
    ))}
      ...
      <div className="grid grid-cols-8 gap-1.5">
        {emojis.map((e) => (
          <button
            ...
            className="text-[20px] flex items-center justify-center h-9 rounded-xl hover:bg-[#c8705a]/[0.10] transition"
          >
            {e}
          </button>
        ))}
  ...
  <div className="absolute bottom-full mb-2 left-0 bg-[#111318] border border-[#c8705a]/[0.18] rounded-2xl shadow-2xl overflow-hidden z-50 w-[180px]">
    {[
      ...
      <button
        ...
        className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[#c8705a]/[0.07] transition text-gray-300 text-[13px]"
      >
...
const SearchInChat = ({
  ...
  const results = q.trim()
    ? messages.filter(
        (m) =>
          !m.deletedAt && m.content.toLowerCase().includes(q.toLowerCase()),
      )
    : [];
  return (
    <div className="absolute inset-x-0 top-0 bg-[#0a0c10] border-b border-[#c8705a]/[0.10] z-40 px-3 py-2.5">
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={onClose}
          className="text-[#c8705a] text-sm font-semibold px-1"
        >
          Done
        </button>
      </div>
        ...
        <div className="mt-2 max-h-[40vh] overflow-y-auto [&::-webkit-scrollbar]:hidden">
          {results.map((m) => (
            <div
              ...
              className="flex items-center gap-2 px-2 py-2.5 hover:bg-[#c8705a]/[0.05] cursor-pointer rounded-xl transition"
            >
              <MessageCircle
                size={13}
                className="text-[#c8705a] flex-shrink-0"
              />
              <p className="text-[13px] text-gray-300 truncate">{m.content}</p>
              <span className="text-[10px] text-[#4a3a35] ml-auto flex-shrink-0">
      ...
      {q && results.length === 0 && (
        <p className="text-[12px] text-[#4a3a35] text-center py-3">
          No results for "{q}"
        </p>
      )}
    </div>
  ...
  return (
    <Modal onClose={onClose}>
      <div className="bg-[#111318] w-full max-w-[340px] rounded-3xl overflow-hidden border border-[#c8705a]/[0.18] shadow-2xl p-6">
        ...
        <p className="text-[13px] text-[#7a6a65] mb-4 truncate">
          "{text.slice(0, 40)}
          {text.length > 40 ? "…" : ""}"
        </p>
        <input
        ...
        <div className="flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl bg-[#c8705a]/[0.08] text-gray-300 text-sm font-semibold hover:bg-[#c8705a]/[0.15] transition"
          >
            Cancel
          </button>
          <button
...
export default function ChatComponent() {
  ...
  const isUserMe = (id: string) => {
    ...
    return (
      nId === normCurrentUserId ||
      nId.endsWith(normCurrentUserId) ||
      normCurrentUserId.endsWith(nId)
    );
  };
  ...
  const [listFilter, setListFilter] = useState<"all" | "unread" | "muted">(
    "all",
  );
  ...
  useEffect(() => {
    if (activeChat?.id) {
      // ✅ Mark as read AND clear the unread badge
      chatHook.markChatAsRead(activeChat.id);
      setActiveChat((prev) => (prev ? { ...prev, unreadCount: 0 } : null));
    }
  }, [activeChat?.id, chatHook]);
  ...
  const sendVoiceNote = useCallback(async () => {
    ...
    await messageHook.send(
      `🎤 Voice message (${formatDuration(recordSec)})`,
      undefined,
    );
    setRecordSec(0);
    chatHook.refresh();
  ...
  const handleSend = useCallback(async () => {
    ...
    await messageHook.send(
      text,
      replyingTo ? { replyToId: replyingTo.id } : undefined,
    );
    setReplyingTo(null);

    // ✅ Clear badge on sender's side after sending
    setActiveChat((prev) => (prev ? { ...prev, unreadCount: 0 } : null));
  ...
  const handleReact = useCallback(
    (msgId: string, emoji: string) => {
      setMsgReactions((prev) => {
        ...
        return {
          ...prev,
          [msgId]: { ...existing, [emoji]: count > 0 ? 0 : 1 },
        };
      });
      setMenuMsgId(null);
      setReactionMsgId(null);
  ...
  const filteredChats = useMemo(() => {
    ...
    if (search.trim())
      chats = chats.filter((c) =>
        resolveChatName(c, currentUserId)
          .toLowerCase()
  ...
  if (view === "chat_room" && activeChat) {
    ...
    return (
      ...
      >
        {/* ── Top bar ── */}
        <div
          className="flex items-center gap-2 px-3 py-2.5 backdrop-blur-xl border-b z-30 sticky top-0"
          style={{
            background: "rgba(10,12,16,0.92)",
            borderColor: "rgba(200,112,90,0.10)",
          }}
        >
          <button
            onClick={() => {
            ...
            onMouseEnter={(e) =>
              (e.currentTarget.style.background = "rgba(200,112,90,0.08)")
            }
            onMouseLeave={(e) =>
            ...
            <div className="min-w-0">
              <div className="text-[15px] font-bold text-white truncate flex items-center gap-1">
                ...
                {activeChat.isVerified && (
                  <BadgeCheck
                    size={13}
                    className="text-[#e8967e] flex-shrink-0"
                  />
                )}
              </div>
              <div
                ...
                {isTyping
                  ...
                  : activeChat.isOnline
                    ...
                    : activeChat.lastMessageAt
                      ? `Active ${timeAgo(activeChat.lastMessageAt)} ago`
                      : "Offline"}
              </div>
            </div>
          </button>
        ...
        {pinnedMsg && (
          <PinnedBanner
            ...
            onJump={() => jumpToMessage(pinnedMsg.id)}
          />
        )}
        ...
        >
          {messageHook.loading && messageHook.messages.length === 0 && (
            <div className="flex justify-center py-20">
              <Loader2 size={22} className="animate-spin text-[#c8705a]" />
            </div>
          )}
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-5 select-none">
              <div className="relative">
                ...
                <div
                  ...
                  style={{
                    background: "linear-gradient(135deg, #c8705a, #a0503a)",
                  }}
                >
              ...
              <button
                ...
                style={{
                  ...
                  borderColor: "rgba(200,112,90,0.30)",
                  color: "#c8705a",
                }}
              >
          ...
          {grouped.map((item, idx) => {
            ...
            const status: MessageStatus = isOptimistic
              ? "sending"
              : msg.isRead
                ? "read"
                : "delivered";
            ...
            return (
              <div
                key={msg.id}
                ref={(el) => {
                  if (el) msgRefs.current[msg.id] = el;
                }}
              ...
              >
                ...
                {replyMsg && !isDeleted && (
                  <div
                    ...
                    style={{
                      background: isMe
                        ? "rgba(200,112,90,0.08)"
                        : "rgba(255,255,255,0.04)",
                      borderColor: isMe
                        ? "rgba(200,112,90,0.18)"
                        : "rgba(255,255,255,0.06)",
                    }}
                  >
                    <div
                      className={`w-0.5 rounded-full self-stretch min-h-[18px] flex-shrink-0`}
                      style={{ background: isMe ? "#c8705a" : "#4a3a35" }}
                    />
                    <div className="min-w-0">
                      <p
                        className="text-[10px] font-bold mb-0.5"
                        style={{ color: isMe ? "#c8705a" : "#7a6a65" }}
                      >
                        {/* FIX 2: Show "You" if reply is from current user, else show the OTHER person's name */}
                        {isUserMe(replyMsg.senderId) ? "You" : displayName}
                      </p>
                      <p className="text-[11px] text-[#4a3a35] truncate">
                ...
                >
                  ...
                  {/* Avatar for both sent AND received messages */}
                  <div className="w-7 flex-shrink-0 mb-1">
                    {isLast ? (
                      isMe ? (
                        ...
                        // Other person's avatar for received messages
                        <Avatar
                          src={activeChat.avatarUrl}
                          name={displayName}
                          size={26}
                  ...
                  {/* Bubble */}
                  <div
                    className={`relative group max-w-[280px] sm:max-w-[330px] ${hasReactions ? "mb-4" : ""}`}
                  >
                    {/* Star indicator */}
                    {isStarred && !isDeleted && (
                      <div
                        className={`absolute top-1 ${isMe ? "left-1" : "right-1"} z-10`}
                      >
                        <Star
                          size={8}
                    ...
                    >
                      {isDeleted ? (
                        <span className="flex items-center gap-1.5">
                          <EyeOff size={12} className="text-[#7a6a65]" />{" "}
                          Message deleted
                        </span>
                      ) : isVoice ? (
                        <VoiceNotePlayer
                      ...
                      {!isDeleted && !isVoice && (
                        <span
                          className={`inline-flex items-center gap-1 ml-2 float-right mt-[5px] text-[10px] leading-none ${
                    ...
                    {hasReactions && !isDeleted && (
                      <ReactionBadge reactions={reactions} isMe={isMe} />
                    )}
                    ...
                    {showReact && !isDeleted && (
                      <div
                        ...
                        style={{
                          background: "#111318",
                          borderColor: "rgba(200,112,90,0.18)",
                        }}
                      >
                        {REACTIONS.map((r) => (
                          <button
                    ...
                    {showMenu && !isDeleted && (
                      ...
                      >
                        {/* Quick reactions row */}
                        <div
                          className="flex items-center justify-around px-2 py-2 border-b"
                          style={{
                            background: "#0f1116",
                            borderColor: "rgba(200,112,90,0.08)",
                          }}
                        >
                        ...
                        {menuActions.map((action) => (
                          <button
                            ...
                            onMouseEnter={(e) =>
                              (e.currentTarget.style.background =
                                "rgba(200,112,90,0.06)")
                            }
                            onMouseLeave={(e) =>
                              (e.currentTarget.style.background = "transparent")
                            }
                          >
                            <span className="text-[#7a6a65]">
                              {action.icon}
                            </span>
                            {action.label}
                          </button>
        ...
        {showScrollBtn && (
          <button
            ...
            style={{
              background: "#111318",
              borderColor: "rgba(200,112,90,0.22)",
            }}
          >
            ...
            {chatHook.chats.find((c) => c.id === activeChat.id)?.unreadCount ? (
              ...
              >
                {
                  chatHook.chats.find((c) => c.id === activeChat.id)
                    ?.unreadCount
                }
              </span>
            ) : null}
        ...
        {/* ── Input area ── */}
        <div
          className="backdrop-blur-xl border-t px-3 pb-5 pt-2 z-20"
          style={{
        ...
        >
          ...
          {(replyingTo || editingMsg) && (
            <div
              className="flex items-center gap-3 rounded-2xl px-4 py-2.5 mb-2 border"
              style={{
                background: "#111318",
                borderColor: "rgba(200,112,90,0.18)",
              }}
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  ...
                  style={{ color: editingMsg ? "#f59e0b" : "#c8705a" }}
                >
                  {editingMsg
                    ? "Editing message"
                    : `Reply to ${isUserMe(replyingTo?.senderId || "") ? "yourself" : displayName}`}
                </p>
                <p className="text-[12px] text-[#7a6a65] truncate">
          ...
          {isRecording ? (
            <div
              className="flex items-center gap-3 rounded-2xl px-4 py-3 mb-2 border"
              style={{
                background: "rgba(200,112,90,0.07)",
                borderColor: "rgba(200,112,90,0.20)",
              }}
            >
              <div className="w-2.5 h-2.5 rounded-full animate-pulse flex-shrink-0 bg-[#c8705a]" />
              <div className="flex-1">
                <p className="text-[13px] text-[#e8967e] font-semibold">
                  ...
                  {Array.from({ length: 20 }).map((_, i) => (
                    <div
                      ...
                      style={{
                        ...
                        background: "rgba(200,112,90,0.40)",
            ...
            <div className="flex items-end gap-2">
              ...
              <div className="relative">
                ...
                >
                  {/* <Plus
                    size={20}
                    className={`transition-transform duration-200 ${showAttach ? "rotate-45 text-rose-400" : "text-[#c8705a]"}`}
                  /> */}
                </button>
                {showAttach && (
                  <div onClick={(e) => e.stopPropagation()}>
                    <AttachmentMenu
                      ...
                      onSelect={(type) =>
                        showToast(`${type} attachment — coming soon`)
                      }
                    />
                  </div>
                )}
              ...
              {/* Input */}
              <div
                className="flex-1 flex items-end gap-2 rounded-[24px] px-4 py-2.5 border transition-colors min-h-[46px]"
                style={{
                  background: "#111318",
                  borderColor: "rgba(200,112,90,0.14)",
                }}
                onFocus={() => undefined}
              >
                <input
                  ...
                  placeholder={
                    editingMsg ? "Edit message…" : `Message ${displayName}…`
                  }
              ...
              {msgInput.trim() ? (
                <div className="flex gap-1.5 flex-shrink-0">
                  ...
                  <button
                    ...
                    style={{
                      background: "linear-gradient(135deg, #c8705a, #9a4e3a)",
                      boxShadow: "0 4px 16px rgba(200,112,90,0.30)",
                    }}
                  >
                    {messageHook.sending ? (
        ...
        {showChatInfo && (
          <ChatInfoModal
            ...
            onClearHistory={() => showToast("Chat history cleared")}
          />
        )}
        {deletingId && (
          <DeleteModal
            ...
            forEveryone={isUserMe(
              messageHook.messages.find((m) => m.id === deletingId)?.senderId ||
                "",
            )}
          />
        )}
        ...
        {schedulingMsg && (
          <ScheduleModal
            text={schedulingMsg}
            onSchedule={(time) => {
              showToast(`📅 Scheduled for ${new Date(time).toLocaleString()}`);
              setMsgInput("");
            }}
  ...
  return (
    ...
    >
      {/* ── Header ── */}
      <div
        className="px-4 pt-4 pb-0 sticky top-0 z-20"
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {dmCreating && (
              <Loader2 size={18} className="animate-spin text-[#c8705a]" />
            )}
            <button
              ...
              style={{
                background: "linear-gradient(135deg, #c8705a, #9a4e3a)",
                boxShadow: "0 4px 14px rgba(200,112,90,0.30)",
              }}
            >
        ...
        {/* Search */}
        <div
          className="flex items-center gap-2.5 rounded-2xl px-4 py-2.5 border mb-3 transition-colors"
          style={{
            background: "#111318",
            borderColor: "rgba(200,112,90,0.12)",
          }}
        >
          ...
          {search && (
            <button onClick={() => setSearch("")}>
              <X size={14} className="text-[#7a6a65]" />
            </button>
          )}
        </div>
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-[12px] font-semibold capitalize transition-all ${
                listFilter === f
                  ...
                  : "text-[#7a6a65] border hover:text-gray-300"
              }`}
              style={
                listFilter === f
                  ? {
                      background: "linear-gradient(135deg, #c8705a, #9a4e3a)",
                      boxShadow: "0 2px 10px rgba(200,112,90,0.25)",
                    }
                  : {
                      background: "#111318",
                      borderColor: "rgba(200,112,90,0.14)",
      ...
      >
        {chatHook.error && (
          <Err msg={chatHook.error} retry={chatHook.refresh} />
        )}
        ...
        {!chatHook.loading && dmChats.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 px-8 text-center gap-5 select-none">
            ...
            <div>
              <p className="font-bold text-[17px] text-white mb-1.5">
            ...
            <button
              ...
              style={{
                background: "linear-gradient(135deg, #c8705a, #9a4e3a)",
                boxShadow: "0 4px 20px rgba(200,112,90,0.30)",
              }}
            >
  ...
  // ✅ Trust the backend: unreadCount already excludes the current user's own messages
      ...
      <div className="flex-1 min-w-0">
        ...
        {!chatHook.loading && search && filteredChats.length === 0 && (
          <div className="px-4 py-12 text-center">
            <p className="text-[14px] text-[#4a3a35]">
