...
// Dark gradients for text-only content area
const CONTENT_GRADIENTS = [
    "linear-gradient(145deg, #0d1b2a 0%, #1a2744 100%)",
    "linear-gradient(145deg, #1a0d2e 0%, #2d1560 100%)",
    "linear-gradient(145deg, #0f1923 0%, #1c2e3d 100%)",
    "linear-gradient(145deg, #12110f 0%, #2a2520 100%)",
    "linear-gradient(145deg, #0d1f0d 0%, #162e1a 100%)",
];

const getGradient = (id?: string) => {
    if (!id) return CONTENT_GRADIENTS[0];
    return CONTENT_GRADIENTS[id.charCodeAt(id.length - 1) % CONTENT_GRADIENTS.length];
};

// Fixed card dimensions
const CARD_HEIGHT = 280;       // px — total card height (collapsed)
const HEADER_HEIGHT = 56;      // px — avatar + name + timestamp row
const ACTION_HEIGHT = 44;      // px — bottom action bar
const CONTENT_HEIGHT = CARD_HEIGHT - HEADER_HEIGHT - ACTION_HEIGHT; // 180px for the body area

const ProfilePlaceholder = ({ size = 36 }: { size?: number }) => (
    <div
        className="bg-gradient-to-br from-[#C9115F] to-[#e8185a] rounded-full flex items-center justify-center text-white flex-shrink-0"
        style={{ width: `${size}px`, height: `${size}px` }}
    >
...
// Splits text into plain-text and clickable URL segments
    ...
    return parts.map((part, i) =>
        URL_REGEX.test(part) ? (
            <a
                ...
                className="text-[#C9115F] underline underline-offset-2 break-all hover:opacity-80 transition-opacity"
            >
                {part}
            </a>
        ) : (
...
export default function PostCard({
    ...
    const hasMedia = displayedMedia && displayedMedia.length > 0;
    const hasPoll  = localPoll && localPoll.options && localPoll.options.length > 0;

    // ── Detect text overflow to conditionally show "show more" ────────────
    useEffect(() => {
        const el = textRef.current;
        if (!el) { setIsOverflowing(false); return; }
        // Temporarily remove clamp to get true scrollHeight
        const prev = el.style.cssText;
        el.style.cssText = "overflow:visible;display:block;-webkit-line-clamp:unset;";
        const overflows = el.scrollHeight > (hasMedia ? 36 : el.parentElement!.clientHeight - 16);
        el.style.cssText = prev;
        setIsOverflowing(overflows);
    }, [displayedContent, hasMedia]);
        ...
        (async () => {
            try {
                const res = await axios.get(
                    `/api/comments?contentId=${post.id}&contentType=post&limit=1`,
                    { signal: controller.signal }
                );
                const total =
                    res.data?.pagination?.total ??
                    (Array.isArray(res.data?.comments) ? res.data.comments.length : null);
                if (!canceled && typeof total === "number") setLocalCommentCount(total);
            } catch { /* ignore */ }
        })();
        return () => { canceled = true; controller.abort(); };
    ...
    const handlePreference = async (action: "suggest_more" | "suggest_less") => {
        ...
        try {
            ...
            setPreferenceToast(
                res.data?.message ??
                (action === "suggest_more" ? "You'll see more posts like this." : "You'll see fewer posts like this.")
            );
            setTimeout(() => setPreferenceToast(null), 3000);
        } catch (err) {
            console.error("Failed to save preference:", err);
    ...
    // ── RENDER ─────────────────────────────────────────────────────────────
    return (
        // Outer wrapper: fixed height always; grows to auto when expanded
        <div
            className="rounded-2xl overflow-hidden border border-white/10 bg-[#1a1a1e] flex flex-col transition-all duration-300"
            style={{ height: expanded ? "auto" : `${CARD_HEIGHT}px` }}
        >

            {/* ── Repost / Quote banner (tiny, outside fixed layout) ── */}
            {(post.isQuoteRepost && post.quotedPost) && (
                <div className="flex items-center gap-1.5 px-3 pt-2 text-white/40 text-[10px] flex-shrink-0">
                    <Quote className="w-2.5 h-2.5" />
                    <span>{post.userName} quoted a post</span>
                </div>
            )}
            {(post.isRepost && !post.isQuoteRepost) && (
                <div className="flex items-center gap-1.5 px-3 pt-2 text-green-400/70 text-[10px] flex-shrink-0">
            ...
            >
                <div className="flex items-center gap-2.5 min-w-0">
                    <div className="w-9 h-9 rounded-full border-2 border-white/15 flex-shrink-0 overflow-hidden">
                    ...
                    <div className="min-w-0">
                        ...
                        <p className="text-white/40 text-[11px] leading-tight mt-0.5">
                            {formatTimeAgo(displayedCreatedAt)}
                        </p>
                    </div>
                </div>
                ...
                {currentUserId && (
                    <div className="relative flex-shrink-0" ref={menuRef}>
                        <button
                            onClick={() => setShowMenu((v) => !v)}
                            disabled={deleting}
                            className="w-7 h-7 rounded-full hover:bg-white/10 transition-colors flex items-center justify-center"
                            aria-label="Post options"
                        >
                        ...
                        {showMenu && (
                            <div className="absolute right-0 top-9 z-30 bg-[#1a1a1a] rounded-xl shadow-2xl border border-white/10 overflow-hidden min-w-[210px]">
                                {isOwner ? (
                                    <button
                                        onClick={handleDelete}
                                        disabled={deleting}
                                    ...
                                    <>
                                        <button onClick={openReportModal} className="w-full px-4 py-3 text-left text-sm text-red-400 hover:bg-red-500/10 transition-colors flex items-center gap-2.5">
                                            <Flag className="w-4 h-4 shrink-0" />
                                            Report
                                        </button>
                                        <div className="h-px bg-white/8 mx-3" />
                                        <button
                                            onClick={() => handlePreference("suggest_more")}
                                            disabled={preferenceAction === "suggest_more"}
                                            className={`w-full px-4 py-3 text-left text-sm transition-colors flex items-center gap-2.5 ${preferenceAction === "suggest_more" ? "text-[#C9115F] bg-[#C9115F]/10 cursor-default" : "text-white/70 hover:bg-white/10"}`}
                                        >
                                            <ThumbsUp className="w-4 h-4 shrink-0" />
                                            Suggest more like this
                                        </button>
                                        <button
                                            onClick={() => handlePreference("suggest_less")}
                                            disabled={preferenceAction === "suggest_less"}
                                            className={`w-full px-4 py-3 text-left text-sm transition-colors flex items-center gap-2.5 ${preferenceAction === "suggest_less" ? "text-white/50 bg-white/5 cursor-default" : "text-white/70 hover:bg-white/10"}`}
                                        >
                                            <ThumbsDown className="w-4 h-4 shrink-0" />
                                            Suggest less like this
                                        </button>
            ...
            {/* ── BODY — fills remaining fixed space, clips overflow ── */}
            {/*
                Height calculation:
                - collapsed: CONTENT_HEIGHT (180px) — clips overflow
                - expanded:  auto — grows freely
                The content inside can be: gradient+text, media, poll, quoted post
            */}
            <div
                className="flex-1 px-3 overflow-hidden transition-all duration-300"
                style={expanded ? { height: "auto", overflow: "visible" } : { height: `${CONTENT_HEIGHT}px` }}
            >

                {/* TEXT-ONLY card: centered text on gradient bg */}
                {!hasMedia && !hasPoll && displayedContent && (
                    <div
                        className="w-full h-full rounded-xl flex items-center justify-center px-4 py-4"
                        style={{ background: getGradient(post.id) }}
                    >
                        <p
                            ref={textRef}
                            className="text-white text-sm leading-relaxed text-center w-full"
                            style={expanded ? {} : { overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 5, WebkitBoxOrient: "vertical" }}
                        >
                ...
                {/* MEDIA card */}
                {hasMedia && (
                    <div className="w-full flex flex-col gap-2" style={{ height: expanded ? "auto" : `${CONTENT_HEIGHT}px` }}>
                        {/* Image fills a fixed block — never shrinks, text never overlaps */}
                        <div
                            className={`relative flex-shrink-0 grid gap-1 rounded-xl overflow-hidden ${displayedMedia!.length === 1 ? "grid-cols-1" : "grid-cols-2"}`}
                            style={{ height: displayedContent ? `${CONTENT_HEIGHT - 40}px` : `${CONTENT_HEIGHT}px` }}
                        >
                            {displayedMedia!.map((item, idx) => (
                                <div key={idx} className="relative bg-zinc-900 w-full h-full">
                                    {item.type === "image" ? (
                                        <Image
                                            src={item.url}
                        ...
                        {displayedContent && (
                            <p
                                ref={textRef}
                                className="text-white/80 text-[12px] leading-snug flex-shrink-0"
                                style={expanded ? {} : { overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}
                            >
                                {renderContent(displayedContent)}
                            </p>
                        )}
                ...
                {post.quotedPost && post.isQuoteRepost && (
                    <div className="w-full h-full flex flex-col gap-2">
                        {displayedContent && (
                            <p className="text-white text-[13px] leading-relaxed flex-shrink-0 line-clamp-2">{renderContent(displayedContent)}</p>
                        )}
                        <div className="rounded-xl border border-white/10 bg-white/5 p-2.5 flex-1 overflow-hidden">
                            <div className="flex items-center gap-1.5 mb-1.5">
                                ...
                                <span className="text-white/25 text-[11px]">{formatTimeAgo(post.quotedPost.createdAt)}</span>
                            </div>
                            {post.quotedPost.content && (
                                <p className="text-white/55 text-[11px] leading-relaxed line-clamp-3">{renderContent(post.quotedPost.content)}</p>
                            )}
                            {post.quotedPost.media && post.quotedPost.media.length > 0 && (
                                <div className="mt-1.5 rounded-lg overflow-hidden bg-black/30 h-14">
                                    <img src={post.quotedPost.media[0].url} alt="quoted media" className="w-full h-full object-contain" />
                                </div>
                            )}
                        </div>
                    </div>
                ...
                {hasPoll && (
                    <div className="w-full h-full flex flex-col">
                        {displayedContent && (
                            <p className="text-white text-[13px] leading-snug mb-2 flex-shrink-0 line-clamp-2">{renderContent(displayedContent)}</p>
                        )}
                        {hasVoted && (
                                    ...
                                    >
                                        <div className="relative bg-white/10 rounded-lg overflow-hidden">
                                            ...
                                            <div className="relative px-3 py-2 flex justify-between items-center">
                                                ...
                                                <span className="text-white/50 text-[10px]">{pct.toFixed(0)}% ({option.votes})</span>
                                            </div>
                                        </div>
                                    </button>
                                );
            ...
            {/* ── SHOW MORE / SHOW LESS — only when text actually overflows ── */}
            {displayedContent && isOverflowing && (
                <div className="px-3 pb-1 flex-shrink-0">
                    <button
                        onClick={() => setExpanded((v) => !v)}
                        className="flex items-center gap-1 text-[#C9115F] text-[11px] hover:opacity-80 transition-opacity"
                    >
                        {expanded ? (
            ...
            {/* ── REACTION SUMMARY — emoji bubbles + count, clickable for modal ── */}
            {localLikes > 0 && (() => {
                const counts: Record<string, number> = {};
                Object.values(localReactions).forEach((r) => { counts[r] = (counts[r] || 0) + 1; });
                ...
                if (topEmojis.length === 0) topEmojis.push("👍");
                return (
                    <button
                        onClick={() => setShowReactionsModal(true)}
                        className="flex items-center gap-1.5 px-3 pb-1.5 flex-shrink-0 group"
                    ...
                    >
                        <div className="flex -space-x-1">
                            {topEmojis.map((emoji, i) => (
                                <span
                                    key={i}
                                    className="text-sm leading-none w-5 h-5 flex items-center justify-center rounded-full bg-white/10 border border-white/5 ring-1 ring-[#1a1a1e]"
            ...
            {/* ── ACTION BAR — fixed height 44px ── */}
            <div
                className="flex items-center justify-between px-4 flex-shrink-0 border-t border-white/[0.07]"
                style={{ height: `${ACTION_HEIGHT}px` }}
            >
                {/* Like + emoji picker */}
                <div
                    className="relative"
                ...
                >
                    ...
                    <button
                        onClick={() => {
                            if (!post.id || !currentUserId) return;
                            if (isLiked) handleReactionToggle(post.id, currentUserId, undefined);
                            else         handleReactionToggle(post.id, currentUserId, "like");
                        }}
                    ...
                    >
                        <Heart
                            className={`w-[18px] h-[18px] transition-all duration-150 ${
                                isLiked
                                    ? "fill-[#C9115F] text-[#C9115F] scale-110"
                                    : "text-white/50 group-hover:text-white/80"
                            }`}
                        />
                        {localLikes > 0 && (
                            <span className={`text-sm tabular-nums ${isLiked ? "text-[#C9115F]" : "text-white/50"}`}>
                                {formatCount(localLikes)}
                            </span>
                        )}
                    </button>
                ...
                >
                    ...
                    {commentCountDisplay !== null && commentCountDisplay > 0 && (
                        <span className="text-sm tabular-nums">{formatCount(commentCountDisplay)}</span>
                    )}
                </button>
                ...
                <button
                    onClick={() => setShowRepostModal(true)}
                    className={`flex items-center gap-1.5 transition-colors ${hasReposted ? "text-green-400 hover:text-green-300" : "text-white/50 hover:text-green-400"}`}
                    title={hasReposted ? "Already reposted" : "Repost or Quote"}
                >
                    <Repeat2 className="w-[18px] h-[18px]" />
                    {localRepostCount > 0 && (
                        <span className="text-sm tabular-nums">{formatCount(localRepostCount)}</span>
                    )}
                </button>
            ...
            {/* ── COMMENTS (below action bar, expands card) ── */}
            {showComments && post.id && (
                <div className="px-3 pb-3 pt-1 border-t border-white/[0.07] flex-shrink-0">
                    <CommentsSection
                        contentId={post.id}
                        contentType="post"
                        contentTitle={post.content?.slice(0, 100) || "Post"}
                            ...
                            <>
                                ...
                                <div className="px-3 pb-3 space-y-1">
                                    {REPORT_REASONS.map((r) => (
                                        <button
                                            key={r.id}
                                            onClick={() => setSelectedReason(r.id)}
                                            className={`w-full flex items-center justify-between px-3 py-3 rounded-xl text-sm transition-colors text-left ${selectedReason === r.id ? "bg-red-500/10 text-white" : "text-white/70 hover:bg-white/5"}`}
                                        >
                                            <span>{r.label}</span>
                                            <span className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ml-3 transition-colors ${selectedReason === r.id ? "border-red-400 bg-red-400" : "border-white/25"}`}>
                                                {selectedReason === r.id && <span className="w-2 h-2 rounded-full bg-white" />}
                                            </span>
                                        </button>
                                    ))}
                                ...
                                <div className="px-4 pb-5 pt-1">
                                    <button
                                        onClick={handleSubmitReport}
                                        disabled={!selectedReason || reporting}
                                        className={`w-full py-3 rounded-xl font-semibold text-sm transition-all ${selectedReason && !reporting ? "bg-white text-[#111] hover:bg-white/90 active:scale-[0.98]" : "bg-white/10 text-white/30 cursor-not-allowed"}`}
                                    >
                                        {reporting ? "Submitting…" : "Report"}
                                    </button>
                                </div>
