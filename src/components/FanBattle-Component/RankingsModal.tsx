...
const Avatar: React.FC<{
  ...
  rank?: number; ringColor?: string; avatarSrc?: string;
  ...
  <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
    {ringColor ? (
      ...
      >
        ...
        >
          {avatarSrc
            ? <img src={avatarSrc} alt={initial} className="w-full h-full object-cover" />
            : initial}
        </div>
      </div>
    ) : (
...
const AthleteRow: React.FC<{ athlete: RankedAthlete }> = ({ athlete }) => {
  ...
  return (
    <div className={`flex items-center gap-3 px-4 py-3.5 border-b border-[#1e1e1e] ${isTop3 ? "bg-[#161616]" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1 mt-0.5">
          <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: athlete.ringColor }} />
          <span className="text-[#666] text-[11px]">{athlete.sport}</span>
        </div>
      </div>
      <div className="flex items-center gap-1 flex-shrink-0">
        <LightningIcon color="#ff6d00" />
        <span className="text-[#ff6d00] text-[14px] font-black">{athlete.points.toLocaleString()}</span>
      </div>
    </div>
  );
};
...
const RankingsModal: React.FC<RankingsModalProps> = ({ isOpen, onClose }) => {
  ...
  // ── Permission check (mirrors PlayerPowerCarousel) ──
  ...
  // ── Fetch leaderboard for a single battle ──
  ...
  // ── Core fetch — identical aggregation logic to PlayerPowerCarousel ──
  const fetchRankings = useCallback(async () => {
    ...
    try {
      ...
      // Collect unique items across all battles
      const itemsMap = new Map<string, {
        id: string; name: string; type: "PLAYER" | "CLUB";
        team: string; avatar?: string;
      }>();

      for (const battle of accessibleBattles) {
        const ids = battle.battleType === "PLAYERS"
          ? battle.selectedPlayers
          : battle.selectedClubs;
        ...
        for (const id of ids) {
          ...
          try {
            if (battle.battleType === "PLAYERS") {
              ...
              itemsMap.set(id, {
                id, name: p.name || "Unknown Player",
                type: "PLAYER", team: p.team || "IPL",
              });
            } else {
              ...
              if (res.data.success) {
                ...
                itemsMap.set(id, {
                  id, name: c.name || "Unknown Club",
                  type: "CLUB", team: c.team || "IPL",
                  avatar: c.avatar,
                });
              }
            }
      ...
      // Aggregate points across all battles
      ...
      // Build ranked list
      const ranked: RankedAthlete[] = Array.from(itemsMap.values())
        .map((item) => ({
          id: item.id,
          name: item.name,
          type: item.type,
          sport: item.team,
          points: pointsMap.get(item.id) || 0,
          avatar: item.avatar,
          avatarColor: colorFromName(item.name, AVATAR_COLORS),
          avatarInitial: item.name.charAt(0).toUpperCase(),
          ringColor: colorFromName(item.name, RING_COLORS),
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetchRankings();
    intervalRef.current = setInterval(fetchRankings, 30_000);
    return () => { if (intervalRef.current) clearInterval(intervalRef.current); };
  }, [isOpen, fetchRankings]);

  if (!isOpen) return null;

  // Lifetime = same aggregated data; swap endpoint here later if needed
  ...
  return (
    <div className="absolute inset-0 z-50 flex flex-col bg-[#121212]">
          ...
          return (
            ...
            >
              {(tab === "live" || tab === "community") && (
                <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 bg-[#00e676] ${isActive ? "animate-pulse" : "opacity-50"}`} />
              )}
              {label}
              {isActive && (
                <span
      ...
      <div className="flex-1 overflow-y-auto">
                ...
                : (
                  <div className="text-center py-12">
                    ...
                    <p className="text-[#555] text-[13px]">
                      {search ? "No players match your search" : "No rankings yet — vote in battles to populate!"}
                    </p>
                  </div>
                )
              }
        ...
        {/* Community tab — static placeholder until API is ready */}
        {activeTab === "community" && (
