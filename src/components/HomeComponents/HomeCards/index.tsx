...
//   console.log("Latest audio files:", latest);
...
function LatestAudioList() {
  ...
  const getDisplayTitle = (audio: typeof audioFiles[0]): string => {
    const title = (audio.title || "").toLowerCase();

    // Always check title string first for known types
    // because matchInfo.type is sometimes "unknown" even for toss reports
    ...
    // Fall back to matchInfo.type only if it's not "unknown"
  ...
  const getSpeakerLabel = (audio: typeof audioFiles[0]): string => {
    const speaker = audio.matchInfo?.speaker || "";
    // Clean up speaker names that have "toss report" or "script" prefix
    return speaker
