...
//         if (hasCountedListen.current || !audioDrop?.id) return;
//         hasCountedListen.current = true;
//         try {
//             const res = await axios.post("/api/plays", { id: audioDrop.id });
//             if (res.data.success) {
...
//         // Reset listen counter when audio changes
...
//             setPlaying(false);
//             setElapsed(0);
//             incrementListens(); // ← fire when audio finishes
//         });
                ...
                <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={closeShareDialog}>
                    <div className="bg-[#1a1a1e] rounded-2xl max-w-md w-full overflow-hidden" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="p-5">
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-3">
                                {[
                                    { label: "WhatsApp", img: "/images/share_whatsapp.png", action: handleShareToWhatsApp },
                                    { label: "Threads", img: "/images/share_thread.png", action: handleShareToThreads },
                                    { label: "Instagram", img: "/images/share_insta.png", action: handleShareToInstagram },
                                    { label: "Copy link", img: "/images/share_copy_link.png", action: handleCopyLink },
                            ...
                            {copied && <p className="text-sm text-emerald-400 mb-3">Copied to clipboard</p>}
                            <button onClick={closeShareDialog} className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-medium text-white hover:bg-white/10 transition">
                                Close
                            </button>
                        </div>
                    </div>
