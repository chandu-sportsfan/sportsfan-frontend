...
//                 <span className="text-[11px] text-white text-[var(--text-muted)]">· {fmt(liveCount)} joined</span>
//               </div>
//             </div>
//           </div>
...
//                 nochanceCount: m.noChanceCount || 0,
//                 heartCount: m.heartCount || 0,
//                 // FIX Bug 3: userLiked comes from the server (per-user boolean).
//                 // Previously this was never mapped, so the icon used heartCount > 0
//                 // which was true for ALL users whenever anyone had liked.
//                 userLiked: m.userLiked ?? existing?.userLiked ?? false,
//                 replyCount: Math.max(m.replyCount ?? 0, existing?.replyCount ?? 0),
//                 agreeCount: m.agreeCount ?? 0,
//                 disagreeCount: m.disagreeCount ?? 0,
//                 userVote: m.userVote ?? null,
...
//           nochanceCount: 0,
//           heartCount: 0,
//           userLiked: false, // FIX: always false for a brand-new post
//           replyCount: 0,
//           agreeCount: 0,
//           disagreeCount: 0,
//           userVote: null,
...
//   /* ── fire / noChance reactions (not heart) ──────────────────────────────────
//    * Optimistic update with rollback on failure.
//    * Heart is handled separately by handleLike() below.
//    */
...
//   /* ── FIX: heart like with optimistic toggle + rollback ──────────────────────
//    *
//    *  Bug 1 fixed — optimistic update fires instantly, UI responds before API.
//    *  Bug 2 fixed — checks post.userLiked and toggles; repeated taps undo like.
//    *  Bug 3 fixed — icon colour driven by post.userLiked (per-user), not
//    *                heartCount > 0 (which was true for every user if anyone liked).
//    */
...
//         { reaction: "heart" }
//       );
//       // Sync server-authoritative count if the API returns it
...
//                     <QuizCard post={p} onToast={onToast} onPostClick={onPostClick} roomId={roomId} />
//                   </motion.div>
//                 );
//               }
...
//                     <div style={{ marginTop: 2 }}>
//                       <div style={{ display: "flex", gap: 16, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 10 }}>
//                         {/* FIX Bug 1 + 2 + 3: handleLike() instead of react("heart"),
//                             colour from p.userLiked not p.heartCount > 0 */}
//                         <button
...
//                           onClick={e => {
//                             e.stopPropagation();
//                             // FIX Bug 1 + 2: heart pill now goes through handleLike()
//                             if (r === "heart") {
//                               handleLike(p.id);
//                             } else {
//                               react(p.id, r);
...
//                     {/* Like / Comment / Share row */}
//                     <div style={{ display: "flex", gap: 16, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 10 }}>
//                       {/* FIX Bug 1 + 2 + 3: handleLike(), colour from p.userLiked */}
//                       <button
//                         onClick={e => { e.stopPropagation(); handleLike(p.id); }}
//                         style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: p.userLiked ? "#ff4a7d" : "#9494ad", fontSize: 13, fontWeight: 600 }}
//                       >
...
export default function DiscussionRoom({
  ...
  const listRef = useRef<HTMLDivElement>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);
  // FIX: tracks posts with an in-flight or recently completed like/unlike so
  // the 3-second poll doesn't overwrite the optimistic count before Firestore
  // has settled — preventing the 10 → 11 → 10 → 11 flicker.
      ...
      if (res.data?.success) {
        setPosts(prev => {
          ...
          return [...res.data.messages]
            ...
            .map((m: any) => {
              const existing = prevMap[m.msgId];
              // FIX: if a like operation is still pending for this post, keep
              // the local optimistic values instead of overwriting with the
              // potentially stale server values from the poll.
              ...
              return {
                ...
                fireCount: m.fireCount || 0,
                nochanceCount: m.noChanceCount || 0,
                // FIX: keep local optimistic heartCount while pending, use
                // server value once Firestore has settled (after 3s buffer).
                heartCount: isPending
                  ? (existing?.heartCount ?? m.heartCount ?? 0)
                  : (m.heartCount ?? 0),
                // FIX: same guard for userLiked — prevents the heart icon
                // toggling back to unfilled during the poll window.
                userLiked: isPending
  ...
  /* ── heart like with optimistic toggle + rollback ───────────────────────────
   *
   * Bug 1 fixed — optimistic update fires instantly, UI responds before API.
   * Bug 2 fixed — checks post.userLiked and toggles; repeated taps undo like.
   * Bug 3 fixed — icon colour driven by post.userLiked (per-user), not
   *               heartCount > 0 (which was true for every user if anyone liked).
   * Bug 4 fixed — pendingLikes ref blocks the 3s poll from overwriting the
   *               optimistic count before Firestore has settled, eliminating
   *               the 10 → 11 → 10 → 11 flicker.
   */
  const handleLike = async (id: string) => {
    if (!roomId) return;
    ...
    // Lock this post against poll overwrites for 3s (matches poll interval)
    ...
    try {
      ...
      // Sync server-authoritative count once API responds
      if (res.data?.heartCount !== undefined) {
        setPosts(p => p.map(x => x.id !== id ? x : {
          ...x,
          heartCount: res.data.heartCount,
      ...
      // Release the lock after 3s — enough time for Firestore to settle so
      // the next poll returns the correct value instead of a stale one.
