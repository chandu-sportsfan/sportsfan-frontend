...
//   // ── LOCAL LIKES STATE ─────────────────────────────────────────────────────
//   // Keyed by post id. Takes priority over props so the UI updates in the same
//   // frame as the click — no waiting for parent setDbPosts → prop drill cycle.
//   // const [localLikes, setLocalLikes] = useState<
//   //   Record<string, { liked: boolean; count: number }>
//   // >({});

//   // Clear local overrides whenever fresh server data arrives so stale local
//   // state can't override a server correction.
//   // useEffect(() => {
//   //   if (dbPosts.length > 0) {
//   //     setLocalLikes({});
//   //   }
...
//       if (!local) return;
//       // ✅ Only clear when the server has CONFIRMED the liked state matches.
//       // If p.userLiked is still false (stale), keep our local override.
//       if (
//         local.liked === (p.userLiked ?? false) &&
//         local.count === (p.likeCount ?? 0)
//       ) {
...
//   // ── RENDER CARD ACTIONS ───────────────────────────────────────────────────
//   // Reads from localLikes first so the heart and count update in <16ms on
//   // click, before the parent's prop-drill cycle completes.
//   const renderCardActions = (item: any) => {
//     const likeOverride = localLikes[item.id];
//     const userLiked = likeOverride !== undefined ? likeOverride.liked : (item.userLiked ?? false);
//     const likeCount = likeOverride !== undefined ? likeOverride.count : (item.likeCount ?? 0);
...
//           onClick={(e) => {
//             e.stopPropagation();
//             // ── Instant local update — UI responds this frame ──
//             setLocalLikes((prev) => ({
//               ...prev,
//               [item.id]: {
//                 liked: !userLiked,
//                 count: Math.max(0, likeCount + (userLiked ? -1 : 1)),
//               },
//             }));
//             if (onLike) onLike(item.id);
//           }}
...
export default function HomeFeed({
  ...
  useEffect(() => {
    ...
    setLocalLikes((prev) => {
      ...
      dbPosts.forEach((p) => {
        ...
        // Only clear when the server has CONFIRMED the liked state matches.
        if (
          local.userLiked === (p.userLiked ?? false) &&
          local.likeCount === (p.likeCount ?? 0)
        ) {
          delete next[p.postId];
        }
      });
  ...
  // Sync votes and likes from dbPosts, but respect recent user actions
  useEffect(() => {
    if (dbPosts.length > 0) {
      const syncedVotes: Record<string, boolean | null> = { ...votes };
      const syncedLikes: Record<string, { userLiked: boolean; likeCount: number }> = { ...localLikes };
        ...
        <button
          onClick={(e) => {
            ...
            setLocalLikes((prev) => ({
              ...
              [item.id]: {
                userLiked: !userLiked,
                likeCount: Math.max(0, likeCount + (userLiked ? -1 : 1)),
              },
            }));
