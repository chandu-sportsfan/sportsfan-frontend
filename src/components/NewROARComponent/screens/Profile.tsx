...
interface Props {
  ...
  setOnboarded?: (b: boolean) => void;
  onNavigateTab?: (tab: string) => void;
  viewingProfile?: string | null;  // ← NEW: For viewing other users
  onClose?: () => void;            // ← NEW: To close profile view
}
...
export default function Profile({ 
  ...
  setOnboarded, 
  onNavigateTab,
  viewingProfile,  // ← NEW: Username being viewed
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (res.data?.success) {
          setProfileData(res.data);
          // Only update badge/editable fields if it's the user's own profile
          if (!viewingProfile) {
            if (res.data.user?.badge) setUserBadge(res.data.user.badge);
            if (res.data.user?.username) setEditName(res.data.user.username);
            if (res.data.user?.favPlayer !== undefined) setEditFavPlayer(res.data.user.favPlayer || "");
          ...
          if (!viewingProfile) {
            // Only handle 404 for own profile
          }
        }
      } catch (err: any) {
        console.error(err);
    ...
    fetchProfile();
  }, [viewingProfile, setUserBadge, setOnboarded, onToast, onClose]);
  ...
  return (
    <div className="screen-scroll">
      {/* ── Header ── */}
      {viewingProfile ? (
        // Viewing another user's profile - custom back button
        <div style={{ 
          padding: "14px 16px", 
      ...
      <div style={{ padding: "24px 16px 0", textAlign: "center", position: "relative" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", position: "relative", width: 96, margin: "0 auto" }}>
          ...
          {/* Pencil edit button — top-right of avatar (only for own profile) */}
          {isOwnProfile && (
            <button
              onClick={() => setAvatarPickerOpen(true)}
              aria-label="Change avatar"
