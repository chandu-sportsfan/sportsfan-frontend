...
const WORDS = [
  "the", "and", "of", "to", "in", "a", "that", "it", "he", "was", "for", "on", "are", "as", "with",
  "his", "they", "be", "at", "one", "have", "this", "from", "or", "had", "by", "but", "not", "what",
  "all", "were", "we", "when", "your", "can", "said", "there", "use", "an", "each", "which", "she",
  "do", "how", "their", "if", "will", "up", "about", "out", "many", "then", "them", "these", "so",
  "some", "her", "would", "make", "like", "into", "him", "has", "two", "more", "very", "after",
  "words", "called", "just", "where", "most", "know", "get", "through", "back", "much", "go", "good",
  "new", "write", "our", "me", "man", "too", "any", "day", "same", "right", "look", "think", "also",
  "around", "another",
];
...
export default function BookCricket() {
  ...
  const resolveFlip = (nextPage: number, result: Result) => {
    ...
    if (result === "OUT") {
      setMessage(
        `Page ${nextPage} -> digit ${nextPage % 10} -> wicket. ${nextWickets < maxWickets ? "Keep batting." : "All out."}`,
      );
    } else if (result === 6) {
  ...
  return (
    <div className="fantasy-book-game">
      <style jsx global>{`
        ...
        .fantasy-book-game .brand-back:hover {
          transform: translateY(-1px);
          border-color: rgba(245, 158, 11, 0.6);
          color: #f5d5a0;
        }

        .fantasy-book-game .brand-text {
          text-align: right;
          flex: 1;
        }
        ...
        .fantasy-book-game .mainBtn {
          ...
          border: 1px solid rgba(196, 64, 16, 0.35);
          border-radius: 999px;
          letter-spacing: 0.22em;
          cursor: pointer;
      ...
      <div className="shell">
        ...
        <div className="stage">
          <div className={`bookScene ${phase === "idle" ? "idle" : ""} ${phase === "flipping" ? "flipping" : ""}`}>
            ...
            <div className={`cover ${bookOpen ? "open" : ""}`} onClick={openBook} role="button" tabIndex={0}>
          ...
          <div className="ballRow">
            {Array.from({ length: maxBalls }, (_, index) => {
              ...
              const className = result
                ? `ballPill ${result === "OUT" ? "out" : `r${Math.min(result as number, 6)}`}`
                : `ballPill ${isCurrent ? "now" : ""}`;
              const content = result ? (result === "OUT" ? "W" : result === 0 ? "•" : String(result)) : String(index + 1);
