...
//         try {
//             // First, upload all files for each question
...
//                             type: q.type,
//                             answer: null,
//                             fileUrls: fileUrls, // Store uploaded URLs here
//                         };
//                     }
//                     return {
//                         questionId: q.id,
...
//             // Upload screenshot attachments separately
...
//                 rating: rating,
//                 attachments: screenshotUrls, // This is for the main attachments/screenshots
...
//             const response = await axios.post("/api/feedback/submissions", submissionData);
...
//                                                 className={`w-full text-left px-4 py-2.5 rounded-xl border text-sm transition ${selected
...
export default function FeedbackForm() {
    ...
    // FIXED: Use your API route for uploads
    const uploadFiles = async (questionId: string): Promise<string[]> => {
        const questionFiles = files[questionId];
        if (!questionFiles || questionFiles.length === 0) return [];
        ...
        for (const file of questionFiles) {
            ...
            try {
                // Using your existing /api/upload route
                ...
                if (res.data.success && res.data.url) {
                    console.log(`Uploaded ${file.name}: ${res.data.url}`);
                    uploaded.push(res.data.url);
                } else {
                    console.error(`Upload failed for ${file.name}:`, res.data.message);
                }
            } catch (err) {
        ...
        console.log(`Uploaded ${uploaded.length} files for ${questionId}`);
        return uploaded;
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            // Upload screenshot attachments using your API route
            const screenshotUrls = await uploadFiles("__screenshots__");
            ...
            console.log("Submitting feedback with attachments:", screenshotUrls.length);
