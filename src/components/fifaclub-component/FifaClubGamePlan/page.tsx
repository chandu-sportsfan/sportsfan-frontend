// components/FifaClub-Component/FifaClubGamePlan/index.tsx
"use client";
...
export default function FifaClubGamePlan({ club }: Props) {
  const [activeTab, setActiveTab] = useState<Tab>("Drops");

  // Derive tactical strengths from club stats
  const winRate = club.matches_played
    ? Math.round((club.wins / club.matches_played) * 100)
    : 0;
  const goalsPerGame = club.matches_played
    ? club.goals_for / club.matches_played
    : 0;

  const strengths: string[] = [];
  if (winRate >= 65)                    strengths.push("Dominant Force");
  if (winRate >= 50 && winRate < 65)    strengths.push("Consistent Performer");
  if (goalsPerGame >= 2.5)              strengths.push("High-Scoring Attack");
  if (goalsPerGame >= 1.5)              strengths.push("Reliable Finishers");
  if (club.goal_difference > 100)       strengths.push("Elite Goal Difference");
  if (club.goal_difference > 50)        strengths.push("Strong Goal Ratio");
  if (club.goals_against < 60 && club.matches_played > 50) strengths.push("Solid Defence");
  if (club.world_cup_apps >= 20)        strengths.push("Tournament Veterans");
  if (club.world_cup_apps >= 15)        strengths.push("Experienced Campaigners");
  if (club.fifa_rank <= 5)              strengths.push("World-Class Squad");
  if (club.fifa_rank <= 10)             strengths.push("Elite Ranking");
  if (club.wins >= 60)                  strengths.push("Record Winning History");
  if (club.all_time_best_finish?.includes("Title")) strengths.push("World Champions");
  if (strengths.length === 0)           strengths.push("Tournament Competitor", "International Pedigree");
    ...
    <div className="flex flex-col gap-4 px-4 md:px-6 pb-18">
      ...
      <div className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-4 md:p-5">
        ...
        <div className="mt-4 space-y-3">
          {[
            { label: "Win Rate",  pct: winRate,                               color: "#16a34a" },
            { label: "Draw Rate", pct: club.matches_played ? Math.round((club.draws / club.matches_played) * 100)  : 0, color: "#d97706" },
            { label: "Loss Rate", pct: club.matches_played ? Math.round((club.losses / club.matches_played) * 100) : 0, color: "#dc2626" },
          ].map(({ label, pct, color }) => (
            <div key={label}>
