...
export default function WT20ClubProfileActions({ club }: Props) {
  ...
  const storageKey = `wt20-club-actions:${club.club_id}`;
  ...
  const handleShareToWhatsApp = () => {
    const text = `Check out ${club.country}'s WT20 WC profile\n${buildShareUrl()}`;
  ...
  // Key overview stat tiles
  const overviewStats = [
    { value: club.won,        label: "WON" },
    { value: club.lost,       label: "LOST" },
    { value: club.tied_so,    label: "TIED/SO" },
    { value: club.matches,    label: "MATCHES" },
  ];
  ...
  return (
    <div className="flex flex-col gap-4 px-4 md:px-6 pt-4 pb-4">
      ...
      {/* Toast */}
      {toastMessage && (
        <div className="rounded-2xl border border-[#0d9488]/30 bg-[#042f2e] px-4 py-3 text-sm font-medium text-white">
          {toastMessage}
        </div>
      )}
          ...
          <div className="relative w-[90%] max-w-md bg-[#111] border border-[#2a2a2a] rounded-xl p-6 z-10">
            ...
            <div className="mt-4 flex gap-3 justify-end">
              ...
              <button onClick={confirmUnfollow}
                className="px-4 py-2 rounded-full text-white font-bold" style={{ background: ACCENT }}>
                Unfollow
              </button>
            </div>
          </div>
      ...
      <div className="flex items-center gap-2 md:gap-3">
        <button onClick={handleFollowClick}
          className="flex flex-1 items-center justify-center gap-2 h-[46px] md:h-[52px] rounded-full text-white text-[14px] md:text-base font-bold border-0 cursor-pointer hover:opacity-90 transition-opacity"
        ...
        <button onClick={toggleWatch}
          className={`flex flex-1 items-center justify-center gap-2 h-[46px] md:h-[52px] rounded-full text-[14px] md:text-base font-bold cursor-pointer transition-colors ${
            isWatching
              ? "border text-white"
              : "bg-transparent border text-[#2dd4bf] hover:bg-[#0d9488]/10"
          }`}
          style={{ borderColor: ACCENT, ...(isWatching ? { background: `${ACCENT}22` } : {}) }}>
        ...
        <button onClick={() => { setShowShareDialog(true); setCopied(false); }}
          className="flex items-center justify-center w-[46px] h-[46px] md:w-[52px] md:h-[52px] rounded-full bg-transparent border text-[#2dd4bf] cursor-pointer shrink-0 hover:bg-[#0d9488]/10 transition-colors"
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setShowShareDialog(false)}>
            <div className="bg-[#1a1a1e] rounded-2xl border border-white/10 p-4 w-[300px] shadow-2xl" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-3">
                <p className="text-white text-sm font-semibold">Share WT20 Club Profile</p>
              ...
              {copied && <p className="text-xs text-teal-400">Copied to clipboard</p>}
            </div>
          </div>
        </>
      )}
      ...
      {/* Add to Watchlist */}
      <button className="flex w-full items-center justify-center gap-2 h-12 md:h-[52px] rounded-full bg-transparent border text-[#2dd4bf] text-[13px] md:text-sm font-bold cursor-pointer hover:bg-[#0d9488]/10 transition-colors"
        style={{ borderColor: ACCENT }}>
      ...
      <div className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-4 md:p-5">
        <SectionLabel text="Match Record" />
      ...
      <div className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl p-4 md:p-5">
        ...
        <div className="grid grid-cols-2 gap-3 mt-3">
          <div className="flex flex-col justify-center p-3.5 md:p-4 rounded-xl bg-[#242424]">
            <span className="text-[20px] md:text-[24px] font-extrabold text-[#2dd4bf] leading-tight">
              #{club.icc_ranking}
            </span>
          ...
          <div className="flex flex-col justify-center p-3.5 md:p-4 rounded-xl bg-[#242424]">
            ...
            <span className="text-[11px] md:text-xs text-[#777777] mt-1">Rating Points</span>
          </div>
          <div className="flex flex-col justify-center p-3.5 md:p-4 rounded-xl bg-[#242424]">
            <span className="text-[20px] md:text-[24px] font-extrabold text-[#4ade80] leading-tight">
              {Math.round(club.win_pct * 100)}%
            </span>
            <span className="text-[11px] md:text-xs text-[#777777] mt-1">Win Rate</span>
          </div>
          <div className="flex flex-col justify-center p-3.5 md:p-4 rounded-xl bg-[#242424]">
            <span className="text-[18px] md:text-[20px] font-extrabold text-white leading-tight">
              {club.apps}
            </span>
            <span className="text-[11px] md:text-xs text-[#777777] mt-1">WT20 Apps</span>
          </div>
        </div>
      </div>
    </div>
